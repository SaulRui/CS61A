...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if hasattr(insect,'queen_id') and insect.queen_id == 1:
                return
            else:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            for bee in list(self.place.bees): # a insect has an atrribute place,
                                              # while a place has attribute ants and bees
                bee.reduce_armor(3)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.queen_id != 1:
            ...
            (before, later) = (self.place, self.place)
            while before is not None:
                if before.ant:
                    ant_set.add(before.ant)
                    if hasattr(before.ant,'ant') and before.ant.ant is not None:
                        ant_set.add(before.ant.ant)
                before = before.exit
            while later is not colony.hive:
                if later.ant:
                    ant_set.add(later.ant)
                    if hasattr(later.ant,'ant') and later.ant.ant is not None:
                        ant_set.add(later.ant.ant)
                later = later.entrance

            for ant in ant_set:
                if ant not in enhanced_ant:
                    if hasattr(ant,'damage'):
                        ant.damage *= 2
                        enhanced_ant.add(ant)
        # END Problem 9
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 9
        Ant.reduce_armor(self, amount)
        if self.queen_id == 1 and self.armor == 0:
            bees_win()
        # END Problem 9
